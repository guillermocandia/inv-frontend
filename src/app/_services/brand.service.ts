...
export class BrandService {
  ...
  get(id: string): Observable<Brand> {
    ...
    return this._http.get<Brand>(url)
    .pipe(
      tap(_ => console.log(`${this.constructor.name}: get ${url}`)),
      catchError(this.handleError<Brand>(`${this.constructor.name}: get ${url}`))
    );
  }

  save(brand: Brand) {
    let url: string;
    const body = {name: brand.name, active: brand.active};
